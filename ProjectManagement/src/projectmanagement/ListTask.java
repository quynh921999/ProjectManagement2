...
public class ListTask extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        infoTask.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mousePressed(java.awt.event.MouseEvent evt) {
                infoTaskMousePressed(evt);
            }
        });
        ...
        infoTaskAL.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mousePressed(java.awt.event.MouseEvent evt) {
                infoTaskALMousePressed(evt);
            }
        });
        ...
        filter.setBackground(new java.awt.Color(51, 255, 255));
    ...
    public int getIDProj(String name){
        ...
        public int getIdTask(String name){
        ...
        public void updateTaskStatus(){
            ...
            int id = getIdTask(name);
            String status = null;
            if(nameDev.equals(null))
                status = "Created";
            else
                status = "Created,Assigned";
            String sql = "insert into status(idTask, status_Task, nameTask) values(?,?,?)";
        try {
            login.dbc.ps = login.dbc.conn.prepareStatement(sql);
            login.dbc.ps.setInt(1, id);
            login.dbc.ps.setString(2, status);
            login.dbc.ps.setString(3, name);
            login.dbc.ps.executeUpdate();
            JOptionPane.showMessageDialog(null, "update in status successfully!");
        } catch (SQLException ex) {
            Logger.getLogger(ListTask.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public void showListTaskEmpFilter(){
        ...
        if(status.getSelectedItem().toString().equals("Select") && date.getText().equals("")){
            showListTaskEmp();
            return;
        }
             ...
             for(Task task : listTask){
                if(task.getCreatedDate().equals(date.getText()) || task.getStatus().equals(status.getSelectedItem().toString()))
                       newList.add(task);
               }
        }
        ...
        for(int i = 0; i < newList.size(); i++){
            ...
            rows[1] = getNameProject(newList.get(i).getProjectId());
            rows[2] = getNamePerson(newList.get(i).getDeveloperIdS());
            rows[3] = getNamePerson(newList.get(i).getTesterIds());
            rows[4] = newList.get(i).getCreatedDate();
            rows[5] = newList.get(i).getEndDate();
            rows[6] = newList.get(i).getStatus();
            model.addRow(rows);
        }
    }
    private void filterActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_filterActionPerformed
        DefaultTableModel model = (DefaultTableModel) jTable1.getModel();
        model.setRowCount(0);
        showListTaskEmpFilter();
    }//GEN-LAST:event_filterActionPerformed

    private void infoTaskMousePressed(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_infoTaskMousePressed
        try {
            PrintWriter pw = new PrintWriter(new  File("C:\\Users\\quynh\\Documents\\NetBeansProjects\\ProjectManagement\\projectInfo\\taskInformation.csv"));
    ...
    }//GEN-LAST:event_infoTaskMousePressed

    private void jTable1MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jTable1MouseClicked
        DefaultTableModel model = (DefaultTableModel) jTable1.getModel();
        row = new Vector();
        row = (Vector<Object>) model.getDataVector().elementAt(jTable1.getSelectedRow());
        InfoTask info = new InfoTask();
        info.setVisible(true);
        info.setTitle("Information Task");
        this.dispose();
    }//GEN-LAST:event_jTable1MouseClicked

    private void infoTaskALMousePressed(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_infoTaskALMousePressed
        ...
        try {
            PrintWriter pw = new PrintWriter(new File("C:\\Users\\quynh\\Documents\\NetBeansProjects\\ProjectManagement\\projectInfo\\taskActionLog.csv"));
            StringBuilder sb = new StringBuilder();
            while(login.dbc.rs.next()){
                String name = login.dbc.rs.getString("nameTask");
                String status = login.dbc.rs.getString("status_Task");
    ...
    }//GEN-LAST:event_infoTaskALMousePressed
